    ...
    <div class="relative w-full max-w-4xl bg-car-card border-2 border-car-accent rounded-3xl p-6 shadow-2xl flex flex-col space-y-5 text-car-text max-h-[92vh] overflow-y-auto">
      ...
      <div class="flex items-center justify-between pb-4 border-b border-car-border">
        <div class="flex items-center space-x-3">
          ...
          <span class="text-[22px] font-black tracking-wide">缤越 COOL 极客初始化配置向导</span>
        ...
        <div class="flex items-center space-x-3">
          <span class="text-[13px] text-car-sub font-bold">开屏安全与特权就绪检测</span>
          <button 
            v-if="canDismiss"
            @click="closeModal" 
            class="w-9 h-9 rounded-xl bg-car-item border border-car-border flex items-center justify-center hover:bg-car-card text-car-sub hover:text-car-text text-xl font-bold cursor-pointer transition-colors"
          >
            ×
          </button>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div class="bg-car-item border border-car-border rounded-2xl p-5 flex flex-col justify-between space-y-4">
          ...
          <div class="pt-3 border-t border-car-border flex items-center justify-between">
            <span class="text-[13px] font-bold text-car-sub">
              {{ disclaimerAgreed ? '已同意免责条款' : '须确认安全底线' }}
            </span>
            <button 
              @click="agreeDisclaimer"
              class="h-[52px] px-5 rounded-xl border-2 font-black text-[15px] cursor-pointer transition-all shadow-sm"
        ...
        <div class="bg-car-item border border-car-border rounded-2xl p-5 flex flex-col justify-between space-y-4">
          <div class="flex flex-col space-y-2">
            <div class="flex items-center justify-between">
              ...
              <span class="px-2 py-0.5 rounded-full text-[11.5px] font-extrabold border" :class="isStoreFrozen ? 'bg-emerald-500/15 border-emerald-500/40 text-emerald-400' : 'bg-amber-500/15 border-amber-500/40 text-amber-400'">
            ...
            <div class="grid grid-cols-2 gap-2.5 pt-1">
              <div class="p-2.5 bg-car-card rounded-xl border border-car-border flex flex-col justify-between">
                <span class="text-[12.5px] font-black text-car-text">通道 A (推荐)</span>
                <span class="text-[11.5px] text-car-sub font-bold mt-1">一键冻结原厂商店，阻断拦截</span>
              </div>
              <div class="p-2.5 bg-car-card rounded-xl border border-car-border flex flex-col justify-between">
                <span class="text-[12.5px] font-black text-car-text">通道 B (跳过)</span>
          ...
          <div class="pt-3 border-t border-car-border flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
              ...
              <button 
                ...
                class="h-[52px] px-3.5 rounded-xl bg-car-card border border-car-border text-car-text hover:border-car-border-light font-black text-[14.5px] cursor-pointer transition-all shadow-sm"
