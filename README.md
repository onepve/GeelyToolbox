...
2. **全业务链路统一沉淀**：商城所有应用下载、控制台自更新、手机快传文件接收统一落盘于系统 `/sdcard/Download/` 根目录；
3. **老版本平滑自愈迁移**：冷启动时自动扫描并平滑迁移旧版本 `00_车机应用/` 下的历史安装包与语音包，无缝过渡不丢数据。
...
3. **安全隔离防护**：清理过程中底层保护核心系统文件与歌曲，绝不误删车主有效数据。
