            ...
            mainHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if ("door_open".equals(type)) {
                        ...
                        String role = prefs.getString("passenger_voice_role", "princess");
                        if ("female".equals(role)) player.play("door_fr_exit.mp3", "开门请注意后方来车，请带好随身物品");
                        else if ("male".equals(role)) player.play("door_fr_male_exit.mp3", "下车请注意后方来车，别忘了随身物品");
                        else if ("queen".equals(role)) player.play("door_fr_queen_exit.mp3", "女王殿下请慢走，请带好随身贵重物品，注意后方来车");
                        else player.play("door_fr_princess_exit.mp3", "公主请下车，小包包和手机别落下哦，注意后方来车");
                    } else if ("door_fr_leave".equals(type)) {
                        String role = prefs.getString("passenger_voice_role", "princess");
                        if ("female".equals(role)) player.play("door_fr_leave.mp3", "副驾车门已关好，再见，祝您一路顺风");
                        else if ("male".equals(role)) player.play("door_fr_male_leave.mp3", "副驾车门已关好，再见，祝您生活愉快");
                        else if ("queen".equals(role)) player.play("door_fr_queen_leave.mp3", "恭送女王殿下，期待下次为您服务");
