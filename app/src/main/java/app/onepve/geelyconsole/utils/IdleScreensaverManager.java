...
public final class IdleScreensaverManager {
    ...
    private static void doTick() {
        ...
        if (idle < 0) {
            ...
            if (readFailStreak == 3) {
                ...
                AppLogger.w(LOG_MODULE, "闲置时间读取失败（通道A不可用），已自动切换为系统熄屏兜底（通道B）");
            } else if (readFailStreak > 3 && readFailStreak % 20 == 0) {
    ...
    private static long readIdleMs(Context ctx) {
        String dump = null;
        try {
            dump = SystemUtils.executePrivileged(ctx,
                    "dumpsys power | grep -E \"mLastUserActivityTime|mWakefulness=\"");
        } catch (Throwable e) {
     ...
     *
     * 车机 ROM 可能有两种打印形态，此处两者都兼容：
     *  a) 纯数字（SystemClock.uptimeMillis 时间基）：mLastUserActivityTime=12345678
     *  b) 格式化时间串（TimeUtils.formatUptime 渲染的墙钟时间）：mLastUserActivityTime=2026-09-10 16:50:00.123
     */
    static long parseIdleMs(String dump, long nowWallMs, long nowUptimeMs) {
        ...
        if (value.matches("^\\d{5,}$")) {
            try {
                ...
                long idle = nowUptimeMs - v;
                return (idle < 0) ? 0L : idle;
            } catch (Throwable e) {
                return -1L;
            }
        }
