...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "1. 赞赏支持弹窗恢复真实官方微信赞赏码：内嵌车规轻量高保真资产，手机扫码秒级识别赞赏支持\n"
                "2. 内测特权身份默认彻底隐藏：出厂状态完全静默不露痕迹，版本号连续点击过程 0 弹窗骚扰，点满 5 次静默开启测试通道\n"
                "3. 车机深度工具箱 ADB 控制台新增【抓 OTA 地址】：二级向导流程询问前置检查，一键调起原厂升级并提取固件下载直链\n"
                "4. 挡位与模式方形黄金触控重构：文案严格收敛为开关已开/关，右侧一开一试一设规整无变形\n"
                "5. 驻车挡(P)与空挡(N)持久化修复：前后端键名契约 100% 对齐，commit 阻塞落盘断电不丢\n"
                "6. CI 健康自检矩阵 6 大硬门禁全面升级，全生命周期保障 0 死链、0 契约遗漏、0 塌陷"
            ),
            "release_date": today
        }
