...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "1. 架构彻底解耦：四大核心模块（四门门控、电动尾门、换挡状态机、驾驶模式）彻底拆分为独立状态管理器，杜绝通道冲刷与信号覆盖\n"
                "2. 权威硬件串口直出：四门与电动尾门优先锁定底层MCU物理串口报文(91 02 01 b6/b7)，硬件电平毫秒级直达，彻底解决车门有时报有时不报\n"
                "3. 状态机互不干扰：电动尾门独立状态追踪，换挡有人感知状态机独立运作，驾驶模式9位常量独立分发，实现零串扰与零漏报"
            ),
            "release_date": today
        }
