...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "1. 方控与语音总闸卡片闭合包裹重构：彻底消除按键悬浮在外视觉脱节感，规范对齐 FeatureCard 自顶到底包裹与分割线设计\n"
                "2. 运行日志弹窗精简与关闭按钮去重：移除左下角重复的刷新与清除按钮，移除右上角多余关闭，保留控制台右上快捷操作与底部标准关闭\n"
                "3. 桌面迷你胶囊新增【显示电瓶电压】：胶囊显示模式平铺为显示应用名、显示动态暗码、显示电瓶电压三大直选磁贴，实时同步 CAN 底盘真实电压\n"
                "4. 术语与单位全链路统一定义：统一使用“电瓶电压”车规命名，悬浮胶囊切至该模式每 2.5 秒自适应动态跳变刷新"
            ),
            "release_date": today
        }
