...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "1. 驻车挡(P)与空挡(N)播报持久化彻底修复：前后端键名完全统一，彻底解决退出重进变回关闭\n"
                "2. 挡位与模式右侧触控区升级：一开一试一设重构为方形黄金触控组合，开关极简仅显开关已开/关，拒绝撑大变形\n"
                "3. 声效设置二级弹窗全面接入与宽屏排版：左右双栏并排同屏直出小爱 TTS 自定义台词与本地音频绑定，无需翻页\n"
                "4. 功能模式切换播报（智能/舒适/经济/运动）全量增配独立播报开关、试听与声效个性化\n"
                "5. 四门迎宾与电动尾门联动开关全链路打通，前后端状态 100% 实时 commit 物理强刷落盘\n"
                "6. 手机无线快传集成真实高对比 QR Code 二维码，手机相机与微信秒扫秒传\n"
                "7. 应用商店未冻结横幅升级车规高对比纯白纯黑排版，彻底消除浅黄低对比刺眼隐形\n"
                "8. 专车方控图解更新为最新原厂高清控制面板图（SX-0017），①~⑦ 号按键定义工整无错位\n"
                "9. CI 健康自检矩阵全面升级：集成模态注册、键名契约、JSBridge 接口 100% 覆盖等 6 大全生命周期硬门禁"
            ),
            "release_date": today
        }
