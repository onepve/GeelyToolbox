...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "1. CI硬核健康门禁升级至13项：新增车规四大模块独立状态机与MCU物理串口(91 02 01)接线闭环门禁，杜绝后续迭代发生架构退化\n"
                "2. 架构彻底解耦：四门门控、电动尾门、换挡状态机、驾驶模式四大模块全面独立，彻底根除通道冲刷与信号覆盖\n"
                "3. 硬件串口直出：四门与电动尾门直锁MCU物理串口硬件电平，毫秒级响应，彻底解决有时报有时不报"
            ),
            "release_date": today
        }
