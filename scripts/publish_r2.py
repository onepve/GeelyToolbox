...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "本次同步更新：\n"
                "1. 彻底修复点火后换挡、换模式没有语音播报：根除电瓶电压与车速误判，车辆点火换挡与模式信号100%秒级放行\n"
                "2. 状态机开关容灾优化：优先匹配前端绑定的独立语音播报开关，彻底杜绝历史影子键干扰\n"
                "3. 换挡保持干脆利落防抖，精准播报前进挡(D)、倒车挡(R)、驻车挡(P)与空挡(N)\n"
                "4. 驾驶模式严格对齐吉利底层物理真值(智能/舒适/经济/运动)，模式切换即刻发声"
            ),
            "release_date": today
        }
