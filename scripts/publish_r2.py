...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "本次同步更新：\n"
                "1. 彻底解决点火换挡与换模式无声：发电机高压充电(≥13.2V)赋予绝对最高优先级，滤除按键弹起(key=0)误判熄火\n"
                "2. 挡位与驾驶模式多源融合：实装原厂核心服务 mModelGearPos 与 mModelDriverMode 解析，覆盖更多吉利车型\n"
                "3. 根治 ADB 终端与日志导出卡顿：全量日志采集全面异步化，抓取行数精准优化，杜绝界面冻结死锁\n"
                "4. 系统底层维护页面直出【⚡ 导出全量日志 (ZIP)】大磁贴，无需再进弹窗一键即可极速导出"
            ),
            "release_date": today
        }
