...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "本次正式版核心修复与升级：\n"
                "1. 点火启动瞬发秒响：精准识别key=10/14/15等点火就绪状态，彻底消除打火起步前7秒换挡无声盲区\n"
                "2. 换挡武装状态机补齐：补齐N挡切入武装判定，从N挡挂回P挡100%响亮播报，杜绝时有时无\n"
                "3. 熄火与蓝牙唤醒静默铁律：蓝牙靠近/解锁严禁误标点火就绪，静置工况下挡位与模式状态机绝对静默\n"
                "4. 彻底根治驻车挡循环误报：严格过滤1号等非标诊断脏报文，P挡必须且仅当切入有效挡位挂回才允许播报\n"
                "5. 驾驶模式开机首包静默基准：开机首包静默确立基准，彻底消除蓝牙点亮时智能与舒适模式交替误报\n"
