...
def main():
    ...
    if is_beta:
        ...
        meta = {
            ...
            "changelog": (
                "本次正式版核心修复与升级：\n"
                "1. 熄火与蓝牙唤醒静默铁律：蓝牙靠近/解锁严禁误标点火就绪，静置工况下挡位与模式状态机绝对静默\n"
                "2. 彻底根治驻车挡循环误报：严格过滤1号等非标诊断脏报文，P挡必须且仅当切入行车挡挂回才允许播报\n"
                "3. 驾驶模式开机首包静默基准：开机首包静默确立基准，彻底消除蓝牙点亮时智能与舒适模式交替误报\n"
                "4. 商城详情与下载全面修复：修复云端应用目录同步后说明内容空白与下载任务无进度的字段映射Bug\n"
                "5. 倒挡全面回归默认媒体声道：彻底消除系统通知流被倒车雷达死锁挂起与切回N挡滞后爆音问题\n"
                "6. 换挡0ms强制抢占打断：手拨换挡瞬间立刻掐灭上一段旧语音，快切挡位绝不堆叠重音\n"
                "7. Tasker实车规则全面对齐：彻底校准DirveMode=2运动/DirveMode=3经济误报Bug，实装getDrivingMode权威挡位\n"
                "8. 根治挡位错位与循环播报：纠正0x14(R挡)误报驻车挡、0x12(D挡)误报空挡Bug，物理挡位精确按低4位解析\n"
            ),
            "release_date": today
        }
